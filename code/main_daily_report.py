...
def main() -> None:
    ...
    print()
    print(f"{'Total Revenue:':<24}${total_revenue:,.2f}")
    print(
        f"{'Busiest day by revenue:':<24}"
        f"{top_by_revenue['date']} (${top_by_revenue['revenue']:,.2f})"
    )
    print(
        f"{'Busiest day by units:':<24}"
        f"{top_by_units['date']} ({top_by_units['units_sold']} units)"
    )
